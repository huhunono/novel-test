...
def is_truthy_shelf_flag(v: Any) -> Union[bool, None]:
...
def test_reg_bookshelf_add_remove_flow(user_client: UserClient) -> None:
    """
    ...
    resp_in = user_client.query_in_shelf(book_id, allow_redirects=False)
    ...
    added: bool = False
    # 2) main process, try/finally ensures cleanup
    try:
        # 2.1 add to bookshelf
        resp_add = user_client.add_to_bookshelf(book_id, allow_redirects=False)
        body = assert_json_response(resp_add)
        assert_ok_true(body)
        assert body.get("code") == 200
        added = True

        # 2.2 queryIsInShelf — book must be in shelf
        resp_in2 = user_client.query_in_shelf(book_id, allow_redirects=False)
        ...
        # 2.3 listBookShelfByPage must include the bookId (search first 5 pages)
        found: bool = False
        for page in range(1, 6):
            resp_list = user_client.list_bookshelf_by_page(page_num=page, page_size=20)
        ...
        # 2.4 remove from bookshelf
        resp_rm = user_client.remove_from_bookshelf(book_id, allow_redirects=False, timeout=10)
        body_rm = assert_json_response(resp_rm)
        assert_ok_true(body_rm)
        assert body_rm.get("code") == 200

        # 2.5 query -> false
        resp_in3 = user_client.query_in_shelf(book_id, allow_redirects=False, timeout=10)
